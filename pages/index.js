...
export default function Index({ tweet }) {
  return (
    ...
    >
      ...
      <P>
        That means no Twitter embed{' '}
        <Code className="inline">`{`<iframe>`}`</Code>, no JS, no layout and
        scrolling jumps, no slowness, great SEO, great lighthouse scores:
      </P>
      <Score>
      ...
      <H2 data={{ id: 'incremental-static-generation' }}>
      ...
      <P>
        Because it’s static HTML, if Twitter disappears from the internet, you
        have strong guarantees of its high availability, backed by redundant
        storage.
      </P>
      <P>
      ...
      <P>
        The <Code className="inline">`getStaticPaths`</Code> hook allows you to
        give Next.js the set of IDs you want to generate at{' '}
        <strong>build time</strong>.
      </P>
      <P>
        When you define a fallback, you get the power to continue lazily
        building pages <strong>at runtime as well</strong>. Unlike traditional
        Server Side Rendering (SSR), the initial request is always{' '}
        <strong>instant</strong> (the fallback itself is also static), you don’t
        worry about <strong>configuring caching</strong> and the work is only
        done <strong>once globally</strong>.
      </P>
      <Hr />
      <P>Resources:</P>
      <Ul>
